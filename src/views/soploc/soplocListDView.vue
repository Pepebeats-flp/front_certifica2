    ...
    >
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props">
          ...
          <q-btn v-if="sumi || sdiag" color="negative" icon="delete" flat dense @click="onDelete(props.row)" />
        </q-td>
      </template>
    </q-table>
  </div>
...
const exportTable = async () => {
  ...
  for (const solicitud_diagnostico of registros) {
    content.push([
      ...
      contratos.get(buses?.[solicitud_diagnostico.placa_patente]?.[35]) || "",
      ...
      solicitud_diagnostico.dominio === "0" ? "CARROCERIA" : solicitud_diagnostico.dominio === "1" ? "CHASIS" : "CHASIS",
      solicitud_diagnostico.ot_solicitud_detalle,
      solicitud_diagnostico.ot_resolucion_concretada_fecha,
      solicitud_diagnostico.ot_resolucion_concretada_hora,
