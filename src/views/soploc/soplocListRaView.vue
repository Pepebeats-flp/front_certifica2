<template>
  <div>
    <SmartTable
      ...
      row-key="uuid"
      :loading="loading"
      persist-key="soplocra-list"
      search-placeholder="Buscar..."
      empty-icon="construction"
      empty-text="No hay solicitudes de reparación"
    >
      <template #top-left>
      ...
      <template #body-cell-actions="{ row }">
        <q-td>
          ...
          <q-btn v-if="sumi || srepa" color="negative" icon="delete" flat dense @click="onDelete(row)">
            <q-tooltip>Eliminar</q-tooltip>
          </q-btn>
        </q-td>
      </template>
      <template #empty-action>
        <q-btn color="primary" to="/soplocRa/add" label="Crear solicitud" />
      </template>
    </SmartTable>
      ...
      <div class="q-gutter-md row items-start">
        <q-date v-if="!showTime" v-model="fecha_resolucion_concretada" :title="fh_concretada ? useDateFormat(fh_concretada, 'DD/MM/YYYY HH:mm').value : ''" :subtitle="solicitud_uuid" :options="optionsFn" :events="eventsFn" :event-color="eventsCFn" @update:model-value="onDateSelected">
          <q-form @submit="onCEnd()">
            <q-input v-model="causa_origen" label="CAUSA / ORIGEN" dense :rules="[(val) => (val && val.length > 0) || 'Este campo es obligatorio.']" input-class="text-uppercase" autogrow />
...
const columns = [
  ...
  { name: "causa_origen_col", label: "Causa / Origen", field: "causa_origen", align: "center", responsive: ["md", "lg", "xl"] },
...
const exportTable = async () => {
  ...
  const content = [];
  content.push(["UNIDAD NEGOCIO", "UNIDAD SERVICIO", "CONTRATO", "ID", "ESTADO", "TIPO SOLICITUD", "PLACA PATENTE", "KILOMETRAJE", "SISTEMA / COMPONENTE", "NUMERO OT", "FECHA SOLICITUD", "HORA SOLICITUD", "SOLICITANTE", "TALLER / PLANTA", "DOMINIO", "DETALLE SOLICITUD", "FECHA RESOLUCION CONCRETADA", "HORA RESOLUCION CONCRETADA", "RESOLUTOR", "CAUSA / ORIGEN", "DETALLE RESOLUCION", "CAUSA ELIMINACION"]);
  ...
  for (const registro of registros) {
    content.push([registro.unidad_negocio, registro.unidad_servicio, contratos.get(buses?.[registro.placa_patente]?.[35]) || "", registro.uuid, registro.estado === 0 ? "En Proceso" : registro.estado === 1 ? "Finalizada" : registro.estado === 3 ? "Eliminada" : "", registro.tipo_solicitud, registro.placa_patente, registro.km_ejecucion, registro.sistema_componente, registro.ot_numero, registro.ot_solicitud_fecha, registro.ot_solicitud_hora, registro.ot_solicitud_name, registro.ot_taller_planta, registro.dominio === "0" ? "CARROCERIA" : registro.dominio === "1" ? "CHASIS" : "CHASIS", registro.ot_solicitud_detalle, registro.ot_resolucion_concretada_fecha, registro.ot_resolucion_concretada_hora, registro.ot_resolucion_name, registro.causa_origen, registro.ot_resolucion_detalle, delete_map.get(registro.causa)]);
  }
