    ...
    <q-card bordered class="q-ma-md">
      ...
      <q-card-section>
        <q-input
          ...
          :readonly="submited || view" debounce="2000"
        ...
        <table class="result-table full-width">
          ...
          <tbody>
            ...
            <tr class="result-row-defectos">
              ...
              <td class="text-center">{{ total_mediums - curr_mediums }}</td>
              <td class="text-center">{{ total_highs - curr_highs }}</td>
            </tr>
...
const cumplimientoPct = computed(() => {
  const total = total_lows.value + total_mediums.value + total_highs.value;
  if (!total) return 0;
  return Math.round((curr_lows.value + curr_mediums.value + curr_highs.value) / total * 100);
});
const defectosResumen = computed(() => {
  const parts = [];
  const bl = total_lows.value - curr_lows.value;
  const bm = total_mediums.value - curr_mediums.value;
  const bh = total_highs.value - curr_highs.value;
